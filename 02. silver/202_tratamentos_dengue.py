...
# MAGIC %md
# MAGIC ##Tratamentos dados dengue de jan/2020 à abr/2024

# COMMAND ----------

path_delta = 'dbfs:/mnt/dbstoragehnadwxeer7qfg/bronze/dengue_jan2020_abr2024'

df_dengue = spark.read.format('delta').load(path_delta)
...
for mes in meses:
    df_mes = df_dengue_selecao.select(
        col('municipio_resid'),
        col('Ano'),
        lit(mes).alias('mes'),
        col(mes).alias('casos')
